...
func multiply(for numberA: Int, numberB: Int, by multiplier: Int) -> Int {
    if (numberA > numberB) {
        return numberA * multiplier
    } else {
        return numberB * multiplier
    }
}

func multiply(for numberA: Int, by multiplier: Int) -> Bool {
    numberA.isMultiple(of: multiplier)
}

let callNumberA = multiply(for: 10, by: 2)
let callNumberB = multiply(number: 10, by: 2)
let callNumberC = multiply(for: 10, numberB: 12, by: 2)
